...
setup = Setup() #instantiation e.g. Setup(β = 0) - and the rest of defaults defined above
...
discretized_y = tauchen(setup.prod_grid_size,setup.ϱ, setup.σ_ϵ) #see Markov chains slides Y = Ay + ϵ;
#ϵ random 
...
ag.prob_trans #0.416817 when in the lowest income this is prob to stay there; see markov slides
# And the productivity grid
ag.prod_grid

  
# Allocate memory for the relevant arrays
c_EGM           = Array{Float64,3}(undef, setup.a_grid_size,setup.prod_grid_size, 2); #consumption policy function; 2: for young and old
