...
#(R = 1, β = 1, σ = 1, a_0 = 0, y = [5, 2], u = var"#12#18"{var"#log_u#16"}(var"#log_u#16"()), u′ = var"#14#20"())
hh.R #1
hh.β #1
hh.a_0 #0
...
hh_01.u(2) #0.0 argument = consumption
hh_1.u(1) #0.0
hh_2.u(2) #Domain error - investigate
...
max_f, a_optim, ret = NLopt.optimize(opt, [0.1]) #a_optim[1] = 1.5
...
function nlopt_objective_fn(a::Vector, grad::Vector,hh)
    ####### TASK 1:  Fill in the missing parts in the nlopt_objective_fn function #######
        ...
        c_2     = hh.R*a[1]   + hh.y[2] - a[2] #Replace the 0!
        c_3     = hh.R*a[2]   + hh.y[3] - a[3] #Replace the 0!
        c_4     = hh.R*a[3]   + hh.y[4] - a[4] #Replace the 0!
        c_5     = hh.R*a[4]   + hh.y[5] #Replace the 0!
        Lifetime_util = hh.u(c_1) + hh.β*hh.u(c_2) + (hh.β^2)*hh.u(c_3) + (hh.β^3)*hh.u(c_4) + (hh.β^4)*hh.u(c_5)   
 # Replace the 0! (what is a lifetime utility?)
        println("Params, Function ",round.(a,digits=5),", ",round(Lifetime_util,digits=5)) 
        return Lifetime_util 
...
c_1     = hh.R*hh.a_0   + hh.y[1] - a_optim[1] #Replace the 0!
c_2     = hh.R*a_optim[1]   + hh.y[2] - a_optim[2] #Replace the 0!
c_3     = hh.R*a_optim[2]   + hh.y[3] - a_optim[3] #Replace the 0!
c_4     = hh.R*a_optim[3]   + hh.y[4] - a_optim[4] #Replace the 0!
c_5     = hh.R*a_optim[4]   + hh.y[5] #Replace the 0!
...
C.c3/C.c2
C.c4/C.c3
hh.R*hh.β #same number as above - makes sense (patience vs. impatience)
