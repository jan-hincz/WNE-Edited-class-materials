...
# There is a maximum inventory size, $\bar{X}$. -> might be less than demand D
...
function T_operator(v,model)
    ...
    for (d_ind, d) in enumerate(X_vec) #going over all possible levels of demand - exogeneous state variable
        for (x_ind, x) in enumerate(X_vec) #going over all possible levels of inventory - X is an endogeneous state variable
    
            RHS_vec = zeros(length(X_vec))
            for (x_next_ind, x_next) in enumerate(X_vec) #considering all possible levels tomorrow
                
                sold = min(d,x)
                revenue = p * sold - K * (x_next > (x - sold)) - c * (x_next - (x - sold))
                #imo 2 logical statements here with 0 or 1
                if x_next >= x - sold
                    RHS_vec[x_next_ind] = revenue + β * sum( v[x_next_ind,d_next_ind] * pdf(ϕ,d_next_ind-1) for d_next_ind in 1:X_max+1 )
                else #pdf of all possible demands (geometric distribution)
...
(;p, ϕ, X_max, K, c, β, X_vec) = model #in our example D is independed from period to period
...
## other iterative methods - opi 

function Tσ_operator(v,σ_ind,model) #fewer loops, next inventory pinned down using policy, we dont iterate over levels of next inventory
...
function opi(model; tol = 1e-6, maxiter = 1000, max_m = 15) #optimistic policy iteration
...
t_vfi = @time vfi(model) #31 sec
t_opi = @time opi(model) #4 sec - much faster and uses less memory
#GO WITH OPI INSTEAD OF VFI